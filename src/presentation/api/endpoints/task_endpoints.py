...
class TaskEndpoint:
   def __init__(self, task_use_case: ITaskUseCase):
      ...
      self.router.add_api_route("/", self.add_task, methods=["POST"], response_model=ResponseDto[TaskDtos.TaskResponseDto])
      self.router.add_api_route("/{task_id}", self.get_task, methods=["GET"], response_model=ResponseDto[TaskDtos.TaskResponseDto])
      self.router.add_api_route("/", self.get_tasks, methods=["GET"], response_model=ResponseDto[Sequence[TaskDtos.TaskResponseDto]])
      self.router.add_api_route("/", self.edit_task, methods=["PUT"], response_model=ResponseDto[TaskDtos.TaskResponseDto])
      self.router.add_api_route("/{name}", self.remove_task, methods=["DELETE"], response_model=ResponseDto[TaskDtos.TaskResponseDto])
   ...
   def get_tasks(self) -> ResponseDto[Sequence[TaskDtos.TaskResponseDto]]:
   ...
   def remove_task(self, name: str) -> ResponseDto[TaskDtos.TaskResponseDto]:
      response: ResponseDto[TaskDtos.TaskResponseDto] = self.task_use_case.remove_task(name)
      if not response.success:
         raise HTTPException(status_code=404, detail=response.message)
